...
class Py3oReport(models.TransientModel):
    ...
    def _get_parser_context(self, report_ref, model_instance, data):
        report_xml = self.ir_actions_report_id
        context = Py3oParserContext(self.env).localcontext
        context.update(report_xml._get_rendering_context(report_ref, model_instance.ids, data))
    ...
    def _create_single_report(self, report_ref, model_instance, data):
        ...
        with closing(os.fdopen(result_fd, "wb+")) as out_stream:
            ...
            localcontext = self._get_parser_context(report_ref, model_instance, data)
    ...
    def _get_or_create_single_report(self, report_ref, model_instance, data, existing_reports_attachment):
        self.ensure_one()
        ...
        if attachment and self.ir_actions_report_id.attachment_use:
            ...
            with open(report_file, "wb") as f:
                f.write(content)
            return report_file
        return self._create_single_report(report_ref, model_instance, data)
    ...
    def create_report(self, report_ref, res_ids, data):
        ...
        if len(res_ids) > 1 and self.ir_actions_report_id.py3o_multi_in_one:
            reports_path.append(self._create_single_report(report_ref, model_instances, data))
        else:
            ...
            for model_instance in model_instances:
                reports_path.append(
                    self._get_or_create_single_report(
                        report_ref, model_instance, data, existing_reports_attachment
                    )
                )
